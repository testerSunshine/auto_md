...
class ApkInfo():
    ...
    def get_apk_version(self):
        cmd = 'aapt dump badging ' + self.apkpath + '| %s "versionName" '% find_util
        result = ""
        try:
    ...
    def get_apk_name(self):
        cmd = 'aapt dump badging ' + self.apkpath + ' | %s "application-label" '% find_util
        result = ""
        p = subprocess.Popen(cmd, stdout=subprocess.PIPE,
                             stderr=subprocess.PIPE,
                             stdin=subprocess.PIPE, shell=True)
    ...
    def get_apk_pkg(self):
        cmd = 'aapt dump badging ' + self.apkpath + ' | %s "package" '% find_util
        result = ""
        p = subprocess.Popen(cmd, stdout=subprocess.PIPE,
                             stderr=subprocess.PIPE,
                             stdin=subprocess.PIPE, shell=True)
    ...
    def get_apk_activity(self):
        cmd = 'aapt dump badging ' + self.apkpath + ' | %s "launchable-activity" '% find_util
        result = ""
        p = subprocess.Popen(cmd, stdout=subprocess.PIPE,
                             stderr=subprocess.PIPE,
                             stdin=subprocess.PIPE, shell=True)
...
class ADB:
    ...
    def get_screen_resolution(self):
        ...
        out = self.shell(
            "wm size")
        size = out.stdout.read().split(' ')[-1].strip().split('x')
    ...
    def install_app(self, app_file):
        ...
        INSTALL_FAILED_TEST_ONLY	应用是 1111-only 的，但安装时没有指定 -t 参数
    ...
    def get_disk(self):
        ...
        for s in self.shell('df').stdout.readlines():
            if '/mnt/shell/emulated' in s or '/storage/emulated' in s:
                lst = []
                for i in s.split(' '):
                    if i:
                        lst.append(i)
...
if __name__ == "__main__":
    A = ApkInfo('D:\tools\app-t_group_4_5_2-release.apk_4.5.2.apk')
    print A.get_apk_name()
