...
class Ia:
    def __init__(self, all_result_path, device):
        ...
        self.device = device
        self.adb = PO.apkBase.ADB(self.device)
        self.queue = Queue(10)
    ...
    def main(self):
        ...
        if 'nt' == os.name:
            install_file = local_readConfig.getAppiumValue('appPath')
        elif 'posix' == os.name:
            install_file = local_readConfig.getAppiumValue('appPathLinux')
        package_name = PO.apkBase.ApkInfo(install_file).get_apk_pkg()
        threads = []
